...
source("score_interpolation.R")


COHORT_YEAR <- 2024  # Update this each year!
IN_XLSX <- "input/community_cohort_inputs_CCER_scores.xlsx"  # Spreadsheet containing latest data
...
writexl::write_xlsx(FACTORS_MUNI, "muni_ccer.xlsx")
writexl::write_xlsx(FACTORS_CCA, "cca_ccer.xlsx")
...
old_scores_munis <- read_csv("output\\3yr\\cohort_assignments_muni_3yr_2022_2024.csv") |> select(GEOID, old_cohort = COHORT_3YR)
old_scores_ccas <- read_csv("output\\3yr\\cohort_assignments_cca_3yr_2022_2024.csv") |> select(CCA_ID, old_cohort = COHORT_3YR)
...
muni_changes <- comparison_muni |>
  select(name = MUNI, cohort_with_eda = old_cohort, cohort_with_ccer = COHORT_3YR, diff, diff_cat) |>
  filter(diff != 0) |>
  mutate(comm_type = "muni")

cca_changes <- comparison_cca |>
  select(name = CCA_NAME, cohort_with_eda = old_cohort, cohort_with_ccer = COHORT_3YR, diff, diff_cat) |>
  filter(diff != 0) |>
  mutate(comm_type = "cca")

changes <- rbind(muni_changes, cca_changes)

table(comparison_muni$diff)
table(comparison_cca$diff)


sum(changes$diff)

writexl::write_xlsx(changes,"ccer_weights_changes.xlsx")
