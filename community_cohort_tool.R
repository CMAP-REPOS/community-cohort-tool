...
# Compare scores/cohorts against previous assignment (2024, no change to method) ----------------------------
...
#   filter(MUNI %in% c("Chicago", "Joliet", "Aurora", "Elgin", "Waukegan"))
 ...
 ############## PART 5 1/2: COMPARE CHANGE OF SCORES to alternate version

 # Compare scores/cohorts against previous assignment (2025, no change to method) ----------------------------
    # munis only

 altv_muni_csv <- paste0("output/3yr/cohort_assignments_muni_3yr_2023_2025_nochange.csv")
 altv_cca_csv <- paste0("output/3yr/cohort_assignments_cca_3yr_2023_2025_nochange.csv")

 ALTV_SCORES_MUNI <- read_csv(altv_muni_csv, col_types=cols(COHORT_3YR=col_character())) %>%
   rename(
     SCORE_ALT_nochange = WEIGHTED_SCORE_3YR,
     COHORT_ALT_nochange = COHORT_3YR
   ) %>%
   select(MUNI, SCORE_ALT_nochange, COHORT_ALT_nochange)

 COMPARE_MUNI <- MUNI_SCORES_3YR_AVG %>%
   select(GEOID, MUNI, SCORE = WEIGHTED_SCORE_3YR, COHORT = COHORT_3YR) %>%
   left_join(ALTV_SCORES_MUNI, by="MUNI") %>%
   mutate(COHORT_CHG = as.numeric(COHORT) - as.numeric(COHORT_ALT_nochange))

 CHANGED_MUNIS <- COMPARE_MUNI[COMPARE_MUNI$COHORT != COMPARE_MUNI$COHORT_ALT_nochange, c("MUNI", "SCORE", "COHORT", "COHORT_ALT_nochange")]
 CHANGED_MUNIS
 ...
 cat("MUNICIPALITIES")
 lm_muni <- lm(SCORE ~ SCORE_ALT_nochange, data=COMPARE_MUNI)
 cat("Correlation (R-squared):", cor(COMPARE_MUNI$SCORE_ALT_nochange, COMPARE_MUNI$SCORE))
 cat("Linear trend: SCORE_new = ", lm_muni$coefficients[1], " + ", lm_muni$coefficients[2], "*SCORE_old", sep="")
 cat("Root-mean-square error (RMSE):", rmse(lm_muni, COMPARE_MUNI))

 # Plots
 ggplot(COMPARE_MUNI) +
   geom_point(aes(x=SCORE_ALT_nochange, y=SCORE, color=COHORT), alpha=0.6, size=2) +
   geom_abline(intercept=0, slope=1, color="gray", linetype="dashed") +
   geom_abline(intercept=lm_muni$coefficients[1], slope=lm_muni$coefficients[2]) +
   scale_x_continuous(limits=c(0, 100), breaks=seq(0, 100, 20)) +
   scale_y_continuous(limits=c(0, 100), breaks=seq(0, 100, 20)) +
 ...
 ggplot(COMPARE_MUNI) +
   geom_count(aes(x=COHORT_ALT_nochange, y=COHORT, color=COHORT)) +
   scale_size_area(max_size = 20) +
   labs(title="Updated vs. previous cohorts (municipalities)") +
   theme_cmap(gridlines="hv", xlab="Previous cohort", ylab="Updated cohort",
              legend.position="right", legend.direction="vertical") +
 ...
 ggplot(COMPARE_MUNI) +
   geom_histogram(aes(x=COHORT_ALT_nochange, fill="Previous"), stat="count", width=0.4, position=position_nudge(x=-0.2)) +
   geom_histogram(aes(x=COHORT, fill="Updated"), stat="count", width=0.4, position=position_nudge(x=0.2)) +
   #labs(title="Updated vs. previous cohorts (municipalities)") +
   theme_cmap(xlab="Cohort", ylab="Number of municipalities")
...
write.csv(CHANGED_MUNIS, "changemunistemp.csv")
